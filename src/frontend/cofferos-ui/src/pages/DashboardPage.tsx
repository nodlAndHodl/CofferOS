...
import { Activity, Plus, Server, Trash2, Wallet as WalletIcon } from 'lucide-react';
...
function NodeCard({ data }: { data: Dashboard | null }) {
    ...
    <Card className="p-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm">
          {connected ? `${node?.chain} · ${node?.blocks?.toLocaleString()} blocks` : 'Not connected'}
        </span>
      </div>
    </Card>
  );
